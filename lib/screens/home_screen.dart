...
class _HomeScreenState extends State<HomeScreen> {
  ...
  String _appMode = 'business'; // personal ou business
  ...
  Future<void> _loadData() async {
    ...
    _appMode = prefs.getString('app_mode') ?? 'business';
  ...
  bool get isPersonal => _appMode == 'personal';
  ...
  Widget build(BuildContext context) {
    ...
    // Telas diferentes conforme o modo
    final List<Widget> screens = isPersonal
        ? [
            // MODO PESSOAL
            DashboardScreen(
            ...
            TasksScreen(
              ...
              onAddTask: (title, desc, date) {
                setState(() {
                  _tasks.add(Task(id: DateTime.now().toString(), title: title, description: desc, dueDate: date));
                });
                _saveData();
              },
              onEditTask: (id, title, desc, date) {
                setState(() {
                  final old = _tasks.firstWhere((t) => t.id == id);
                  _tasks.removeWhere((t) => t.id == id);
                  _tasks.add(Task(id: id, title: title, description: desc, dueDate: date, isCompleted: old.isCompleted));
                });
                _saveData();
              },
              onToggleTask: (id) {
            ...
            FinancesScreen(
              ...
              isPersonalMode: true,
              onAddTransaction: (type, amount, desc, category, date) {
                setState(() {
              ...
              onEditTransaction: (id, type, amount, desc, category, date) {
                setState(() {
        ...
        : [
            ...
            EmployeesScreen(
              ...
              onAddEmployee: (name, role, phone) {
                setState(() {
                  _employees.add(Employee(id: DateTime.now().toString(), name: name, role: role, phone: phone));
                });
                _saveData();
              },
            ...
            ClientsScreen(
              ...
              onAddClient: (name, notes) {
                setState(() {
                  _clients.add(Client(id: DateTime.now().toString(), name: name, notes: notes));
                });
                _saveData();
              },
    ...
    final titles = isPersonal
        ? ['AdminPro', 'Tarefas', 'Finanças']
        : ['AdminPro', 'Tarefas', 'Funcionários', 'Finanças', 'Clientes'];

    final destinations = isPersonal
        ? const [
